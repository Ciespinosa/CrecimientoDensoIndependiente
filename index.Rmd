...
La dinámica de la población es una de las características más importantes de una población, los cambios en el tamaño poblacional resumen muchos de los procesos que se dan al interior de la comunidad. El tamaño poblacional está asociado a las tasas de natalidad y mortalidad, así como a las emigraciones e inmigraciones. Comprender como la población cambia en el tiempo es fundamental para poder desarrollar acciones de manejo de especies protegidas o en peligro de extinción.

Uno de los modelos más básicos de crecimiento poblacional es el __crecimiento denso-independiente__, este crecimiento básicamente postula que la tasa de incremento _per capita_ no tiene dependencia estadística del tamaño de la población. En otras palabras, las tasas de natalidad y mortalidad de un individuo son independientes de número de individuos que tenga la población. Aunque biológicamente esto parezca que no tiene mucho sentido, el modelo de crecimiento denso-independiente propone que es necesario que aunque haya una influencia esta sea estadísticamente detectable. El poder de detectar una relación significativa depende, en parte, de aquellos factores que gobiernan el poder en las relaciones estadísticas entre dos variables continuas: el número de observaciones y el rango de la variable predictora. Por lo tanto, nuestra conclusión de que una población en particular exhibe un crecimiento independiente de la densidad, puede ser trivial si nuestro tamaño de muestra es pequeño (es decir, pocas generaciones muestreadas), o si muestreamos la población en un rango muy estrecho de densidades. 

#Crecimiento geométrico discreto

En un bosque natural se ha reportado la llegada de una especie de mariposa introducida, es importante realizar el seguimiento de esta población con el fin de evaluar los posibles efectos sobre la diversidad local. Estas mariposas tienen un ciclo de vida de 3 meses, así que se ha decidido muestrear el bosque cada tres meses con el fin de evaluar los cambios en el tamaño poblacional. Luego de 13 meses de seguimiento tenemos datos tamaño poblacional de esta especie en 5 periodos distintos. Vamos a ver que nos dice esta información.
...
Vemos que aparentemente la población está creciendo, aparentemente ese crecimiento es más grande cada vez.  Una forma de ver la tasa a la cual está creciendo la población es dividiendo el tamaño poblacional en el tiempo dos para el tamaño en el tiempo uno. 
...
Como vemos, el crecimiento de la población tiene una tasa de incremento de dos en todos los periodos. No es como pensábamos que la población está creciendo más cada año, la tasa de incremento se ha mantenido constante a lo largo de los 13 meses de seguimiento de la población. Esta tasa implica que la población se duplica cada año, esta aseveración la podríamos marcar como:

$$N_{abril} = 2 * N_{enero}$$

$$\frac{N_{abril}}{N_{enero}} = 2$$

Podemos generalizar esta formula como: 
...
Ahora podemos utilizar la fórmula que derivamos para proyectar la población. Veamos primero la proyección de la población por cada periodo, en el caso del ejemplo de las mariposas cada 3 meses.
...
Como vemos podemos utilizar cualquier momento de la población, únicamente es necesario que lo multipliquemos por tantos ciclos queremos proyectar, así de enero a octubre hay tres ciclos por lo que la tasa de crecimiento deberíamos multiplicarla tres veces: 2 x 2 x 2 o lo que es lo mismo $2^2$, así la tasa se eleva a la cantidad de ciclos que queremos proyectar la población. Ahora podemos generalizar e incluir en nuestra formula como:


$$N_{t+1} = \lambda^t * N_t$$
 
 Veamos si nuestra fórmula nos permite proyectar la población de las mariposas.
...
Efectivamente en 12 periodos pasaremos de 2 a 8192 individuos de mariposas. Con nuestro modelo podríamos predecir el tamaño de la población para los periodos que nos interese. Como vemos el incremento de la población puede ser muy importante, pero ¿qué variables pueden generar cambios en el crecimiento? El tamaño poblacional inicial y la tasa de incremento per capita pueden tener importantes efectos en el crecimiento.

##Efecto del tamaño inicial

Nuestra población de mariposas se está duplicando en cada periodo, esto implica que la magnitud del crecimiento es cada vez mayor, crece de una forma geométrica. Pensemos, una población inicial de 10 individuos se duplica al cabo de un periodo, implicaría que tendríamos 20 individuos, la población se incrementó en 10 individuos. Sin embargo, la duplicación de una población inicial de 20 individuos implicaría un incremento de 20 individuos. Como vemos la magnitud del incremento es diferente dependiendo del tamaño de la población inicial.
...
Como vemos al parecer las poblaciones crecen a ritmos diferentes, la población 1 crece más lentamente que la población 3, y esto se hace más evidente en cada ciclo. Pero realmente las poblaciones están creciendo a una tasa distinta. Para responder esta pregunta podemos log-transformar los datos y observar si la tasa de crecimiento es distinta.   
...
Como podemos ver en esta gráfica las curvas de crecimiento realmente son paralelas, las tres poblaciones están creciendo a una tasa similar. Aunque cambia el intercepto (donde inicia la población) los cambios son lineares en las tres poblaciones.


##Efecto de la tasa de incremento ($\lambda$)

Como hemos visto lambda actúa directamente sobre la población, por lo que los cambios en la tasa _per capita_ tienen un fuerte efecto sobre el crecimiento de la población. Cuando lambda tiene valores inferiores a 1 la población decrece, mientras que con valores superiores a 1 la población se incrementa, con valores de 1 la población se mantiene constante.
...
En este caso la media geométrica calculada como el producto de los valores de lambda para cada periodo y elevada a 1 dividido para t (t es tiempo, el número de periodos) mostró ser mejor medida para proyectar nuestra población.
...
```
Use el siguiente código para hacer los gráficos

plot(sparrows$Year, sparrows$Count, type="l")
lines(sparrows$Year, proy1) #proy1 es la proyección 1
lines(sparrows$Year, proy2) #proy2 es la proyección 2
```
...
```{r, echo=FALSE, fig.cap="Grafica de crecimiento de gorriones en Darrtown. OH. La línea negra muestra el crecimiento real de la población, la línea roja muestra la proyección de la población. Finalmente la línea azul muestra los datos usados para el modelo."}
